...
/*
 * code from: https://github.com/IsaacChanghau/OptimizedImageEnhance/blob/master/src/main/java/com/isaac/utils/Filters.java
 */
...
public class Filters {
   ...
   * @param percent controls the percentage of pixels to clip to white and black.
  ...
  /**
   * Guided Image Filter for grayscale image, O(1) time implementation of guided
   * filter
   *
  ...
  public static Mat GuidedImageFilter(Mat I, Mat p, int r, double eps) {
    ...
    int rows = I.rows();
    int cols = I.cols();
    // N = boxfilter(ones(hei, wid), r); % the size of each local patch; N=(2r+1)^2
    ...
    // cov_Ip = mean_Ip - mean_I .* mean_p; % this is the covariance of (I, p) in
    // each local patch.
    Mat cov_Ip = new Mat();
