...
/*
 * code from: https://github.com/IsaacChanghau/OptimizedImageEnhance/blob/master/src/main/java/com/isaac/estimate/BlkTransEstimate.java
 */
...
public class BlkTransEstimate {
  ...
  public static double blkEstimate(Mat blkIm, double[] airlight, double lambda, double fTrans) {
    ...
    while (nCounter < (1.0 - fTrans) * 10) {
      ...
      double nSumOfSquareOuts = Core.sumElems(bChannel.mul(bChannel)).val[0]
          + Core.sumElems(gChannel.mul(gChannel)).val[0] + Core.sumElems(rChannel.mul(rChannel)).val[0];
      // calculate the value of sum of out
      double nSumOfOuts = Core.sumElems(bChannel).val[0] + Core.sumElems(gChannel).val[0]
          + Core.sumElems(rChannel).val[0];
